  ...
  const fetchDayData = async (date) => {
    ...
    try {
      const [revenue, orders] = await Promise.all([
        AxiosInterceptor.get(`/api/stats/sales-revenue/manager?Start=${formatDateForAPI(startOfDay)}&End=${formatDateForAPI(endOfDay)}`),
        AxiosInterceptor.get(`/api/stats/number-of-seller-orders/manager?Start=${formatDateForAPI(startOfDay)}&End=${formatDateForAPI(endOfDay)}`)
      ]);
  ...
  const fetchWeekData = async (start, end) => {
    try {
      const [gadgets, soldGadgets, weekRevenue, weekOrders] = await Promise.all([
        AxiosInterceptor.get('/api/stats/number-of-gadgets/manager'),
        AxiosInterceptor.get('/api/stats/number-of-sold-gadgets/manager'),
        AxiosInterceptor.get(`/api/stats/sales-revenue/manager?Start=${formatDateForAPI(start)}&End=${formatDateForAPI(end)}`),
        AxiosInterceptor.get(`/api/stats/number-of-seller-orders/manager?Start=${formatDateForAPI(start)}&End=${formatDateForAPI(end)}`)
      ]);
